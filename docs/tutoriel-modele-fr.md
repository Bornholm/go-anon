...
Deux sources sont disponibles selon la couverture souhaitée :

**Option A — fichier national INSEE (~37k prénoms, aucune dépendance externe)**
...
Le fichier national agrège les prénoms rares sous `_PRENOMS_RARES`. Le fichier
régional les ventile par département et offre une couverture plus large.
Télécharger le fichier Parquet (`prenoms-<annee>.parquet`) depuis :
```
...
> stagne autour de 3 % au lieu de converger vers 80 %+.
...
echo "Jean Dupont travaille chez Airbus à Toulouse." \
  | ./bin/demo -lang fr -model model_fr_pruned.crf.gz

# Avec anonymisation
echo "Contactez Marie Martin au 06 12 34 56 78." \
